...
import bi.util.PositionRank;
import bi.util.ProximityCheckB;
...
public class ProximityValidation {
	...
	private static double P_VALUE = 0.005;
	...
	public ProximityValidation(File input, File output, int proximityCount, int proximityThreshold, String genomeFile, String indexFile, boolean buildTree, double mvmCR, String chrLenFile) throws IOException{
		P_VALUE=mvmCR;
		System.out.println("USING pconc="+P_VALUE);
		...
		prox = new ProximityCheckB(proximityThreshold,Integer.MAX_VALUE/10);
...
//			initialiseSearchTree(inFile,false);
			initialiseSearchList(inFile,false);
		}
	}
...
//		initialiseSearchTree(inFile,onlyTakePerfectMatches);
		initialiseSearchList(inFile,onlyTakePerfectMatches);
	}
	
	public void overwriteAnkerLimit(int span){
		prox.ankerThreshold=span;
	}
	...
	private void initialiseSearchList(File file, boolean onlyTakePerfectMatches) throws IOException{
		BufferedReader br = new BufferedReader(new FileReader(file));
		String line;
		System.out.println("\tBuilding Search List");
		long startingTime = System.currentTimeMillis();
		int readCount=0;
		Vector<SAMEntry> allEntries = new Vector<SAMEntry>();
		while((line=br.readLine())!=null){
			if(line.length()<2) continue;
			SAMEntry r1 = new SAMEntry(line);
			...
			if(numberOfReadsInFile > 1000){
				if(readCount % (numberOfReadsInFile/20) == 0){
					double perc = Math.round(50.0*readCount/numberOfReadsInFile);
		...
		System.out.println("\tSorting Search List");
		Collections.sort(allEntries);
		System.out.println("\tFeeding Search List");
		readCount=0;
		for(SAMEntry r1:allEntries){
			readCount++;
			if(allEntries.size() > 1000){
				if(readCount % (allEntries.size()/20) == 0){
					double perc = Math.round(100.0*readCount/allEntries.size());//Math.round(readCount % (testCounter/200));
					System.out.println("\t\t"+perc+"% processed [time = "+(System.currentTimeMillis()-startingTime)+"ms] ...");
				}
			}
			...
			prox.addLinear(new ReadTag(r1.getQname(),file1), r1.getPosition(), transformChromosomeName(r1.getRname()), strand1);
		}
		prox.linkMates();
	}
	...
	private void initialiseSearchTree(File file, boolean onlyTakePerfectMatches) throws IOException{
		...
		while((line=br.readLine())!=null){
			...
			prox.add(new ReadTag(r1.getQname(),file1), new ReadTag(r2.getQname(),file2), r1.getPosition(), r2.getPosition(), 
	...
	private static HashMap<String,String> readTagNameToLinesGlobalTemp;
	public Vector<Cluster> run(boolean printReadNames, boolean onlyTakePerfectMatches, boolean memorizeLines, File summaryFolder) throws IOException{
		File folder = new File(summaryFolder.getParent());
		...
		System.out.println("\t... used file for clustering: "+inFile.getAbsolutePath());
		BufferedReader br = new BufferedReader(new FileReader(inFile));
		BufferedWriter bw = new BufferedWriter(new FileWriter(outFile));
		System.out.println("\t... out1 = "+outFile.getAbsolutePath());
		...
		while((line1=br.readLine())!=null){
			...
			if(!currentReadIsAlreadyPartOfCluster){
				...
				for(ReadTag t:emptyList1){
					int pos=prox.getPositionRank(t.getName()+t.getFileNumber()).getPosition();
			...
			if(emptyList1.size()>=proximityCount || currentReadIsAlreadyPartOfCluster){
				...
				if(minClusterSize>emptyList1.size()) minClusterSize = emptyList1.size();
				emptyList1.removeAllElements();
				emptyList2.removeAllElements();
			}
			test++;
			if(numberOfReadsInFile>100 && test%(numberOfReadsInFile/10)==0) System.out.println(" --> "+(Math.round(100.0*test/numberOfReadsInFile))+"% ("+test+") max_clr="+maxClusterSize+" min_clr="+minClusterSize);
		}
		br.close();
		...
		if(memorizeLines){
			readTagNameToLinesGlobalTemp=readTagNameToLines;
		}
		return clusterPairs;
	}
	...
	private void filterReadPairsAccordingToMvm(BufferedWriter logW) throws IOException{
		...
		while((line1=br.readLine())!=null){
			...
			if(doesValidityFilterFail(r1,r2,proximityThreshold,pairs)){
				...
				filterCount++;
				rejected.write(r1.getRname()+":"+r1.getPosition()+"\t"+r2.getRname()+":"+r2.getPosition()+"\t"+mapVal.lastValue+"\t"+r1.getSequence().length()+"\t"+r2.getSequence().length());
				rejected.newLine();
				continue;
			}
			else{
	...
	public void performMvmFilter(File in, File out, int readLength, double[] span, int threads) throws IOException, InterruptedException{
		...
		if(span==null){
			...
			System.out.println("\tExclude custom range: from "+span[0]+" to "+span[1]+" (both inclusive)");
			ZIndependentMvmFilter.useCustomSimulationRemovalSequence=true;
			ZIndependentMvmFilter.span=span;
			pairedValidityRatioThreshold=-1;
		}
		RLEN=readLength;
		independentMvmFilterMultiThread(in, out, threads);
	}
	...
	public void performClusterAnalysis(int readLength) throws IOException{
		...
		reinitialise(false);
		translocations = run(true,false,true,in1);
		...
		BufferedWriter bw2 = new BufferedWriter(new FileWriter(new File(out1.getAbsolutePath())));
		for(Cluster translocation:translocations){
			...
			if(doesFoundClusterConsistOfEnoughUniqueEvents(translocation)){
				...
				MateCluster cluster1 = translocation.getMate1();
				MateCluster cluster2 = translocation.getMate2();
				if(cluster1.getChromosomeName().equals("hsM") || cluster2.getChromosomeName().equals("hsM")) continue;;	
				for(ReadTag rt:translocation.getMate1().getOccupants()){
					ReadTag rt2=null;
					for(ReadTag rtt:translocation.getMate2().getOccupants()){
						if(rtt.getName().equals(rt.getName())){
							rt2=rtt;
							break;
						}
					...
					bw2.write(readTagNameToLinesGlobalTemp.get(rt.getName()+rt.getFileNumber()));
					bw2.newLine();
					bw2.write(readTagNameToLinesGlobalTemp.get(rt2.getName()+rt2.getFileNumber()));
					bw2.newLine();
				}
			}
		}
	...
	private boolean doesFoundClusterConsistOfEnoughUniqueEvents(Cluster translocation){
		...
		else if(reads1.size()==0){
			System.err.println("ERROR::SIZE==0");
		}
	...
	private double obtainPairedValidityThreshold(double significanceLevel, int readLength) throws IOException{
		...
		System.out.println("\tconcordant input: "+concordantFile.getAbsolutePath());
		File concordantRatioFile = new File(concordantFile.getParent()+"/pairs_remapped_conc.txt");
		boolean generateConcordantDistribution = !concordantRatioFile.exists() || (concordantRatioFile.exists() && concordantRatioFile.length()<300);
		if( generateConcordantDistribution ){
			System.out.println("\tGenerating");
			ValidityMonitoring valmo = new ValidityMonitoring(genomeFile,indexFile, chrLenFile);
			valmo.testRun(concordantFile, concordantRatioFile, readLength);
		}
		...
		System.out.println("\tNumber of used pairs: "+values.size());
		double threshold = getThresholdFromCdf(values,significanceLevel);
		return threshold;
	}
	...
	private MateCluster getNewCluster(Vector<ReadTag> emptyList) {
		...
		String chrName = prox.getPositionRankByName(emptyList.get(0).getName(), emptyList.get(0).getFileNumber()).getChromosomeName();
		for(ReadTag tag : emptyList){
			PositionRank rank = prox.getPositionRankByName(tag.getName(),tag.getFileNumber());
	...
	private boolean doesValidityFilterFail(SAMEntry read1, SAMEntry read2, int outerRange, BufferedWriter bw) throws IOException{
		...
		if(read1.getZM()!=null && read1.getZM()!=null){
			double v1=Double.parseDouble(read1.getZM().split("ZM:Z:")[1]);
			double v2=Double.parseDouble(read1.getZM().split("ZM:Z:")[1]);
			lowerValidity = mapVal.calculateLowerValidityScoreAndPrint(v1, v2, bw);
		}
		else{
